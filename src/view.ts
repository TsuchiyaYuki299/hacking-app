  ...
  <html lang="ja">
    ...
    <body>
      ...
      <div class="terminal hidden" id="main-terminal">
        ...
        <button class="explain-btn" onclick="openModal()">
          [ ? ] 仕組みの解説を見る
        </button>
      </div>

      <dialog id="explain-dialog">
        <h2 style="margin-top:0; border-bottom: 1px solid #00ff00;">
          SECRET FILE: TRUTH
        </h2>
        <h3 style="margin-bottom: 5px; color: #ccffcc;">
          なぜこれで侵入できたの？
        </h3>
        ...
        <p>
          コンピュータはこの命令を左から順に読みますが、最後の部分を見てください。<br />
          <code>OR '1'='1'</code>
        </p>

        <p>
          これは日本語に訳すと<strong>「または、1と1が同じならOK」</strong>という意味です。<br />
          1と1は常に同じ（True）ですよね？
        </p>

        <p>
          つまり、パスワードが合っていようがいまいが、<strong>「または1=1（正解）なら全部OK！」</strong>というルールが追加されたことで、ガードマン（DB）は「あ、条件満たしてるんで通っていいっすよ」と通してしまったのです。
        </p>

        <p
          style="font-size: 0.9rem; color: #88ff88; border-top: 1px dashed #005500; padding-top: 10px;"
        >
          これがSQLインジェクションの正体です。
        </p>
